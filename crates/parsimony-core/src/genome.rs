...
mod tests {
    ...
    #[test]
    fn parses_ecoli_gene_table() {
        let p = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../examples/genome/ecoli_k12_genes.csv");
        if !p.exists() { return; } // generated by scripts/ecoli_genome_csv.py
        let g = Genome::from_csv(&p).expect("parse ecoli gene table");
        assert_eq!(g.length_bp, 4_641_652);
        assert!(g.genes.len() > 4000, "expected >4000 genes, got {}", g.genes.len());
    }
